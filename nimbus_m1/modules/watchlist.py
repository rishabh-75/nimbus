...
def detect_alerts(entries: list[dict]) -> list[dict]:
    """
    Return alert dicts for watchlist symbols needing attention.
    Uses mean-reversion exit logic (not old Mode A BB riding).

    Alert triggers:
    - Price above SMA + BBW contracting → mean reversion complete, take profit
    - Score dropped below 30 → setup deteriorated
    - MFI collapsed below 20 → institutional support gone
    """
    alerts = []
    for e in entries:
        live = e.get("live", {})
        if not live:
            continue
        sym = e["symbol"]
        score = live.get("dm_score", 100)
        tier = live.get("dm_tier", "NONE")
        sizing = live.get("dm_sizing", "SKIP")
        wr = live.get("dm_wr")
        mfi = live.get("dm_mfi", 50)
        pct_sma = live.get("dm_pct_sma", 0)
        bbw_slope = live.get("dm_bbw_slope", 0)

        # BBW contraction exit: above SMA + BBW slope < 0 → mean reversion done
        if pct_sma > 0 and bbw_slope < 0:
            alerts.append({
                "symbol": sym,
                "alert_type": "MR_EXIT",
                "message": f"{sym} — Above SMA + BBW contracting. Mean reversion complete. Take profit.",
                "severity": "warn",
            })
        ...
        if score < 30:
            alerts.append({
                ...
                "message": f"{sym} score dropped to {score}/100. Review position.",
                "severity": "warn",
            })

        # MFI collapsed — institutional support gone
        if mfi < 20:
            alerts.append({
                "symbol": sym,
                "alert_type": "MFI_COLLAPSE",
                "message": f"{sym} MFI={mfi:.0f} — no institutional buying. Consider exit.",
                "severity": "warn",
            })
