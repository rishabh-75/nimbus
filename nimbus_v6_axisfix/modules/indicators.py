...
def add_bollinger(df: pd.DataFrame, period: int = 20,
                  std_dev: float = 1.0, col: str = "Close") -> pd.DataFrame:
    df = df.copy()
...
def add_williams_r(df: pd.DataFrame, period: int = 50,
...
def bb_signal(df: pd.DataFrame) -> dict:
    ...
    if close >= upper:
        ...
        position = "near_lower"
    return dict(
        riding_upper=riding, broke_upper=broke,
        position=position, bb_pct=bb_pct,
        last_close=close, upper=upper, mid=mid, lower=lower,
    )
...
def wr_signal(df: pd.DataFrame, threshold: float = -20.0) -> dict:
    ...
    return dict(
        ...
        wr_value=last_wr, trend=trend, threshold=threshold,
    )
...
def compute_price_signals(
    ...
    try:
        ...
        if not daily.empty and len(daily) >= 20:
            ...
            ps.daily_sma      = float(sma20.iloc[-1]) if not pd.isna(sma20.iloc[-1]) else None
            if ps.daily_sma:
