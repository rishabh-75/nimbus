...
def _analyze_symbol_inner(symbol: str) -> Optional[dict]:
    ...
    riding_upper = bb_upper is not None and not np.isnan(bb_upper) and spot >= bb_upper
    ...
    if has_opts and ctx:
        ...
        gex_regime = (
            "TREND_FRIENDLY" if gex_regime_raw in ("Negative", "Neutral") else "PINNING"
        )

        near_mp = (
            max_pain is not None
            and spot > 0
            and abs(spot - max_pain) / spot < 0.015
            and dte <= 4
        )
    ...
    # ── Viability score (exact weights from spec) ─────────────────────────────
    score = _score(
        riding_upper=riding_upper,
        wr_above_m20=wr_above_m20,
        wr_phase=ps.wr_phase,
        bars_since_m50=ps.wr_bars_since_cross50,
    ...
    # Sizing rules (hard)
    hard_fail = not passes_bias or not wr_above_m20 or not riding_upper
    if hard_fail:
        size = "ZERO"
    elif gex_regime == "PINNING" or expiry_risk == "HIGH" or ps.vol_state == "EXPANDED":
        size = "HALF"
    else:
        size = "FULL"
...
def _score(
    ...
    # ── Momentum (25 pts) ─────────────────────────────────────────────────────
    if riding_upper and wr_above_m20:
        if wr_phase == "FRESH":
            score += 25
            ...
            score += 15  # in zone but unknown phase
    elif riding_upper:
        score += 8  # on upper band but WR not yet > -20
    elif wr_above_m20:
        score += 5  # WR ok but not on upper band

    # ── Regime (20 pts) ───────────────────────────────────────────────────────
    if not has_opts:
        score += 0  # no data — no credit
    elif gex_regime == "TREND_FRIENDLY":
        score += 20
    else:
        score += 0  # PINNING = 0, not negative (penalty applied via hard sizing rule)

    # ── Structure / room to resistance (20 pts) ───────────────────────────────
    if pct_to_resistance is None:
        score += 0
    elif pct_to_resistance >= 8.0:
        score += 20
    elif pct_to_resistance >= 5.0:
        score += 14
    elif pct_to_resistance >= 3.0:
        score += 7
    else:
        score += 0  # too close to wall — no structural credit

    # ── Expiry / pin risk (15 pts) ────────────────────────────────────────────
    if dte >= 99:
        score += 0  # no options data
    elif dte >= 7:
        score += 15
    elif dte >= 5:
        score += 10
    elif dte >= 3 and expiry_risk != "HIGH":
        score += 4
    else:
        score += 0  # DTE 1-2 or HIGH pin risk = no credit

    if near_mp:
        score -= 8  # gravitational pull penalty
