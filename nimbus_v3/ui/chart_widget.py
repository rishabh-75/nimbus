...
def filter_market_hours(df):
    ...
    if hours is None or hours.min() == hours.max():
        return df
...
def _to_ist(df):
    ...
    if idx.tz is not None:
        ...
        idx = idx + pd.Timedelta(hours=5, minutes=30)
...
class VolumeAxisItem(pg.AxisItem):
    """Format volume as 1.9M / 340K instead of 1.9e+06."""
    ...
    def tickStrings(self, values, scale, spacing):
        ...
        for v in values:
            if v >= 1_000_000:
                ...
                result.append(str(int(v)) if v > 0 else "")
        return result
...
class NimbusChart(pg.GraphicsLayoutWidget):
    ...
    def _init_layout(self):
        ...
        self._vol_axis = VolumeAxisItem(orientation="right")
        self._vol_plot = self.addPlot(row=1, col=0, axisItems={"right": self._vol_axis})
        self._cfg(self._vol_plot, show_x=False, y_width=_Y_AXIS_W)

        self._wr_plot = self.addPlot(
            row=2, col=0, axisItems={"bottom": self._date_axis}
        )
        self._cfg(self._wr_plot, show_x=True, y_width=_Y_AXIS_W)
        ...
        for p in (self._vol_plot, self._wr_plot):
            p.setXLink(self._price_plot)
    ...
    def update_data(self, price_df, ctx=None, ps=None, symbol="NIFTY"):
        ...
        if ps and ps.position_state not in ("UNKNOWN", None):
            ...
            lbl = pg.TextItem(
                f" {labels.get(ps.position_state, '')} ",
                color=col,
                anchor=(0, 0),
            ...
            if "High" in df.columns:
                lbl.setPos(0, float(df["High"].max()))
            self._price_plot.addItem(lbl)
        ...
        if "WR" in df.columns:
            ...
            if ok.any():
                ...
                self._wr_plot.addItem(
                    pg.FillBetweenItem(ct, cb, brush=pg.mkBrush(16, 185, 129, 20))
                )
                ct2 = pg.PlotDataItem(xs, np.full(n, -80.0))
                cb2 = pg.PlotDataItem(xs, np.clip(wr, -100, -80))
                self._wr_plot.addItem(
                    pg.FillBetweenItem(ct2, cb2, brush=pg.mkBrush(239, 68, 68, 15))
                )
        ...
        if "High" in df.columns and "Low" in df.columns:
            self._price_plot.setYRange(
                float(df["Low"].min()) * 0.997,
                float(df["High"].max()) * 1.003,
                padding=0.01,
            )
