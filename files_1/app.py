...
from modules.options_wall import OptionsWallCalculator
from modules.insider_detector import InsiderWallDetector
from modules.signal_engine import SignalEngine
from modules.data_manager import DataManager
from modules.alert_manager import AlertManager
from modules.trade_recommender import TradeRecommender
...
def init_session():
    defaults = {
        ...
        "oi_spike_pct": 20.0,
        "entity_filter": ["FII/FPI", "DII/MF", "Promoter/Insider", "Institutional Broker"],
...
def run_compute_pipeline(
    ...
    master_card = engine.build_master_card(
        composite, options_signal, institutional_signal,
        pcr_data["pcr_oi"], max_pain, cmp, summary,
    )
...
with st.sidebar:
    ...
    with col1:
        symbol_input = st.text_input("Symbol", value=st.session_state["symbol"], key="symbol_input").upper()
    with col2:
        recent = st.selectbox("Recent", st.session_state["recent_symbols"], label_visibility="collapsed")
    ...
    wall_pct = st.slider("Wall OI Percentile", 50, 95, st.session_state["wall_pct"], step=5)
    st.session_state["wall_pct"] = wall_pct

    proximity = st.slider("Proximity Tolerance (%)", 0.5, 3.0, st.session_state["proximity_pct"], step=0.1)
    st.session_state["proximity_pct"] = proximity

    min_score = st.slider("Min Signal Score", 0, 100, st.session_state["min_score"], step=5)
    st.session_state["min_score"] = min_score

    st.divider()
    st.markdown("**🏦 Entity Filter**")
    all_entities = ["FII/FPI", "DII/MF", "Promoter/Insider", "Institutional Broker", "Retail/Unknown"]
    selected_entities = []
    ...
    deals_file = st.file_uploader("Bulk/Block Deals CSV", type=["csv"], key="deals_upload")
...
if st.session_state["last_refresh"] is None or manual_refresh or options_file or deals_file or download_nse:
...
tabs = st.tabs(["🏦 Wall Overview", "🕵️ Insider Activity", "🎯 Signal Dashboard", "💡 Trade Ideas", "📊 Historical", "⚙️ Settings"])
...
with tabs[0]:
    if walls_df is None or walls_df.empty:
        ...
        sent_emoji = {"Bullish": "🟢", "Mildly Bullish": "🟡", "Mildly Bearish": "🟠", "Bearish": "🔴"}.get(sentiment, "⚪")

        with k1:
            st.metric("PCR OI", f"{pcr_val:.3f}", help="Put/Call OI ratio. <0.7 bullish, >1.3 bearish")
        with k2:
            st.metric("Sentiment", f"{sent_emoji} {sentiment}", help="Market sentiment derived from PCR")
        with k3:
            st.metric("Max Pain", f"₹{max_pain:.0f}", help="Strike where option writers lose least (OI-weighted)")
        with k4:
            primary_support = key_levels.get("primary_support")
            st.metric("Primary Support", f"₹{primary_support:.0f}" if primary_support else "N/A",
                      help="Highest Put Wall strike = key support floor")
        with k5:
            primary_res = key_levels.get("primary_resistance")
            st.metric("Primary Resistance", f"₹{primary_res:.0f}" if primary_res else "N/A",
...
with tabs[1]:
    if walls_df is None or walls_df.empty:
        ...
        with i1:
            st.metric("Total Deals Today", summary.get("total_deals", 0),
                      help="Total bulk/block deals matched for this symbol today")
        with i2:
            st.metric("At Wall Levels", summary.get("at_wall_levels", 0),
                      help="Deals with signal score ≥ 40 (near a wall strike)")
        with i3:
            st.metric("High Conviction (≥60)", summary.get("high_conviction", 0),
                      help="Deals scoring ≥ 60 / 100")
        with i4:
            st.metric("Top Entity", summary.get("top_entity", "N/A"),
        ...
        with sc_l:
            st.plotly_chart(cb.signal_score_scatter(matched_df), use_container_width=True)
        with sc_r:
            st.plotly_chart(cb.net_qty_bar_chart(aggregated_df), use_container_width=True)
        ...
        if matched_df is not None and not matched_df.empty:
            display_cols = [c for c in [
                "Date", "Client Name", "Entity", "Buy/Sell",
                "Quantity Traded", "Trade Price/Wght. Avg. Price",
                "Wall_Type_Hit", "Nearest_Strike", "Score", "Signal_Strength", "Interpretation"
            ...
            if "Quantity Traded" in tdf.columns:
                tdf["Quantity Traded"] = tdf["Quantity Traded"].apply(lambda x: f"{x:,.0f}")
            ...
            st.download_button("⬇️ Download Deals CSV", csv, "matched_deals.csv", "text/csv")
        else:
            st.info("No matched deals found. Lower the Min Signal Score or adjust proximity tolerance.")
...
""",
        ...
        with p1:
            ...
            opt_color = "#00e676" if opt_score > 0 else ("#ff5252" if opt_score < 0 else "#ffd740")
            st.markdown(f"""
...
<p style="color:{opt_color};font-size:1.3rem;font-weight:700">Score: {opt_score:+d}</p>
        ...
        with p2:
            ...
            inst_color = "#00e676" if inst_score > 0 else ("#ff5252" if inst_score < 0 else "#ffd740")
...
<h4 style="color:#ce93d8">🎯 Composite Conviction</h4>""", unsafe_allow_html=True)
...
Options(40%) + Institutional(40%) + IV(20%)
        ...
        if zones_df is not None and not zones_df.empty:
            ...
            st.info("No zone data available. Load deal data to see institutional zones.")
...
with tabs[3]:
    ...
    if walls_df is None or walls_df.empty:
        ...
        if not setups:
            ...
            k5.metric("Top Conviction",  f"{top.conviction}/100",
            ...
            with fc1:
                dir_filter = st.multiselect(
                    ...
                    default=["LONG", "SHORT"], key="rec_dir"
                )
            with fc2:
                tf_filter = st.multiselect(
                    "Timeframe", ["Intraday", "Swing", "Positional"],
                    default=["Intraday", "Swing", "Positional"], key="rec_tf"
                )
            with fc3:
            ...
            filtered_df = setups_df[
                setups_df["ID"].isin([s.id for s in filtered])
            ] if not setups_df.empty else pd.DataFrame()
...
""",
                ...
                display_cols = [
                    "ID", "Direction", "Strategy", "Timeframe",
                    "Entry Zone", "Target 1", "Target 2", "Stop Loss",
                    "R:R", "Conviction", "Move %", "Risk %", "Signal",
                ]
...
with tabs[4]:
    ...
    if len(history) < 2:
        st.info("📁 Need at least 2 days of options data in ./data/options/ for historical comparison. Demo data shown below.")
        ...
        with col_h1:
            st.plotly_chart(cb.pcr_trend_chart(pd.Series(pcr_vals), dates), use_container_width=True)
        with col_h2:
            st.plotly_chart(cb.max_pain_migration_chart(mp_vals, dates), use_container_width=True)
    else:
        ...
        try:
            calc_today = OptionsWallCalculator(dm._normalise_options(history[today_date]))
            walls_today = calc_today.consolidate_walls()

            calc_yest = OptionsWallCalculator(dm._normalise_options(history[yesterday_date]))
            walls_yest = calc_yest.consolidate_walls()

            # OI Change
            st.plotly_chart(cb.oi_change_chart(walls_today, walls_yest), use_container_width=True)
            ...
            col_h1, col_h2 = st.columns(2)
            with col_h1:
                st.plotly_chart(cb.pcr_trend_chart(pd.Series(pcr_series), dates_list), use_container_width=True)
            with col_h2:
                st.plotly_chart(cb.max_pain_migration_chart(mp_series, dates_list), use_container_width=True)

            # New OI built today
            st.markdown("### 🆕 New OI Built Today (>10% increase)")
            merged = walls_today[["Strike", "Total_OI"]].merge(
                walls_yest[["Strike", "Total_OI"]].rename(columns={"Total_OI": "Prior_OI"}),
                on="Strike", how="inner"
            )
            merged["Change_%"] = (merged["Total_OI"] - merged["Prior_OI"]) / merged["Prior_OI"].clip(lower=1) * 100
            new_oi = merged[merged["Change_%"] > 10].sort_values("Change_%", ascending=False)
            if not new_oi.empty:
                st.dataframe(new_oi.style.background_gradient(subset=["Change_%"], cmap="Greens"),
                             use_container_width=True)
            else:
...
with tabs[5]: # Settings
    ...
    with col_dq1:
        ...
        if options_df is not None and not options_df.empty:
            ...
            st.metric("Strikes", options_df["Strike"].nunique() if "Strike" in options_df.columns else 0)
            st.metric("Expiries", options_df["Expiry"].nunique() if "Expiry" in options_df.columns else 0)
            st.dataframe(
    ...
    with col_dq2:
        ...
        if deals_df is not None and not deals_df.empty:
            ...
            if "Entity" in deals_df.columns:
                st.dataframe(deals_df["Entity"].value_counts().to_frame("Count"), use_container_width=True)
    ...
    preview_tab = st.radio("Show:", ["Options Chain", "Deals", "Walls"], horizontal=True)
    ...
    if lr:
        st.write(f"Last refreshed: {datetime.datetime.fromtimestamp(lr).strftime('%H:%M:%S')}")
        st.write(f"Mode: {'⚠️ DEMO' if st.session_state.get('is_demo') else '✅ Live Data'}")
    ...
    st.info("Auto-refresh every 5 min during market hours (09:15–15:30 IST). Reload page to refresh manually outside market hours.")
