...
def _make_scroll_panel(
    widget: QWidget, height: int = 280, stretch: bool = False
) -> QScrollArea:
...
def _build_dual_checklist(dm_sig, ctx=None):
    ...
    if dm_sig.wr_30 < -50:
        checks.append(
            ("pass", "WR(30) Deep", f"WR={dm_sig.wr_30:.0f}", "Strong bounce expected")
        )
    elif dm_sig.wr_30 < -30:
        checks.append(
            ("pass", "WR(30) Zone", f"WR={dm_sig.wr_30:.0f}", "Mean reversion zone")
        )
    else:
    ...
    if not dm_sig.above_sma and dm_sig.pct_from_sma < -3:
        checks.append(
            ("pass", "Below SMA", f"{dm_sig.pct_from_sma:+.1f}%", "Deep pullback")
        )
    elif not dm_sig.above_sma:
        checks.append(
            ("pass", "Below SMA", f"{dm_sig.pct_from_sma:+.1f}%", "Pullback active")
        )
    else:
        checks.append(
            ("fail", "Above SMA", f"{dm_sig.pct_from_sma:+.1f}%", "Not a pullback")
        )
    ...
    if dm_sig.mfi >= 50:
        ...
        checks.append(
            ("fail", "MFI Weak", f"MFI={dm_sig.mfi:.0f}", "No buying pressure")
        )

    # 4. Drawdown from high
    if dm_sig.dd_from_high <= -10:
        checks.append(
            ("pass", "Deep Drawdown", f"{dm_sig.dd_from_high:.1f}%", "Capitulation")
        )
        ...
        checks.append(
            (
                "pass",
                "Moderate DD",
                f"{dm_sig.dd_from_high:.1f}%",
                "Meaningful pullback",
            )
        )
    else:
        checks.append(
            ("warn", "Shallow DD", f"{dm_sig.dd_from_high:.1f}%", "Minor dip")
        )

    # 5. Red streak
    if dm_sig.red_streak >= 4:
        checks.append(
            ("pass", "Exhaustion", f"{dm_sig.red_streak}d red", "Selling spent")
        )
    elif dm_sig.red_streak >= 2:
        checks.append(
            ("pass", "Red Streak", f"{dm_sig.red_streak}d red", "Sellers tiring")
        )
    else:
        checks.append(
            ("warn", "No Streak", f"{dm_sig.red_streak}d", "No exhaustion signal")
        )

    # 6. Options
    if dm_sig.options_overlay > 3:
        checks.append(
            ("pass", "Options ✓", dm_sig.options_detail, f"+{dm_sig.options_overlay}")
        )
    elif dm_sig.options_overlay < -3:
        checks.append(
            ("fail", "Options ✗", dm_sig.options_detail, f"{dm_sig.options_overlay}")
        )
    ...
    if dm_sig.is_trap:
        ...
        checks.append(
            ("pass", "Filing ✓", dm_sig.filing_detail, f"+{dm_sig.filing_overlay}")
        )
    ...
    if dm_sig.tier == "PRIMARY":
        ...
        missing = []
        if dm_sig.wr_30 >= -30:
            missing.append("WR oversold")
        if dm_sig.above_sma:
            missing.append("below SMA")
        if dm_sig.mfi < 30:
            missing.append("MFI > 30")
        checks.append(
            (
                "fail",
                "NO ENTRY",
                f"Missing: {', '.join(missing)}" if missing else "Not met",
                "Wait",
            )
        )
...
class DashboardTab(QWidget):
    ...
    def _build_ui(self):
        ...
        fl.addWidget(self._dm_mode_badge)
        self._dm_score_lbl = _t("—", MUTED, 9, mono=True, bold=True)
        fl.addWidget(self._dm_score_lbl)
        self._dm_label_badge = Badge("—")
        fl.addWidget(self._dm_label_badge)
        self._dm_sizing_badge = Badge("SIZE —")
        fl.addWidget(self._dm_sizing_badge)
        center.addWidget(fstrip)

        # ── Verdict + entry strip (combined) ──────────────────────────────
        vstrip = QFrame()
        ...
        # Entry reason stored as attribute only (not in layout — used by on_dual_mode_updated)
    ...
    def on_dual_mode_updated(self, symbol: str, dm_sig):
        ...
        # ── Dual-mode strip ───────────────────────────────────────────────
        tier_style = {"PRIMARY": "BULLISH", "SECONDARY": "LATE", "NONE": "NEUTRAL"}.get(
            dm_sig.tier, "NEUTRAL"
        )
        ...
        self._dm_sizing_badge.set_badge(
            f"SIZE: {dm_sig.dual_sizing}", dm_sig.dual_sizing
        )
        ...
        if dm_sig.entry_triggered:
            ...
            parts = []
            if dm_sig.wr_30 >= -30:
                parts.append(f"WR={dm_sig.wr_30:.0f}")
            if dm_sig.above_sma:
                parts.append("above SMA")
            if dm_sig.mfi < 30:
                parts.append(f"MFI={dm_sig.mfi:.0f}")
        ...
        zone = (
            "DEEP" if dm_sig.wr_30 < -50 else ("IN ZONE" if dm_sig.wr_30 < -30 else "—")
        )
        ...
        if dm_sig.sma_20 > 0:
            ...
            all_levels.append(
                ("DAILY SMA", dm_sig.sma_20, sma_lbl_c, f"{dm_sig.pct_from_sma:+.1f}%")
            )
        ...
        if ps:
            if getattr(ps, "upper", 0) > 0:
                ...
                all_levels.append(
                    ("BB UPPER", round(ps.upper, 2), GOLD, f"{pct:+.1f}%")
                )
            if getattr(ps, "lower", 0) > 0:
                pct = (dm_sig.close / ps.lower - 1) * 100 if dm_sig.close > 0 else 0
                all_levels.append(
                    ("BB LOWER", round(ps.lower, 2), GREEN, f"{pct:+.1f}%")
                )
        ...
        if ctx and hasattr(ctx, "walls"):
            ...
            if getattr(w, "resistance", None):
                all_levels.append(
                    ("OI RESIST", w.resistance, RED, f"{w.resistance_pct:+.1f}%")
                )
            if getattr(w, "support", None):
                all_levels.append(
                    ("OI SUPPORT", w.support, GREEN, f"{w.support_pct:+.1f}%")
                )
        ...
        frame_c = (
            sc_c
            if dm_sig.dual_score >= 60
            else GOLD if dm_sig.dual_score >= 45 else RED
        )
        ...
        if dm_sig.tier == "PRIMARY":
            ...
            vc = GOLD
            missing = []
            if dm_sig.dd_from_high > -5:
                missing.append(f"DD={dm_sig.dd_from_high:.1f}% (need <-5%)")
            if dm_sig.red_streak < 2:
                missing.append(f"{dm_sig.red_streak}d red (need ≥2)")
            if dm_sig.vol_ratio < 0.5:
                missing.append(f"Vol={dm_sig.vol_ratio:.1f}x (need ≥0.5)")
            verdict = (
            ...
            if dm_sig.is_trap:
                ...
                missing = []
                if dm_sig.wr_30 >= -30:
                    missing.append(f"WR={dm_sig.wr_30:.0f} (need <-30)")
                if dm_sig.above_sma:
                    missing.append("above SMA")
                if dm_sig.mfi < 30:
                    missing.append(f"MFI={dm_sig.mfi:.0f} (need >30)")
    ...
    def _update_verdict_strip(self, dm_sig):
        ...
        if sc <= 30:
            ...
            ac = GREEN
            opts_warn = ""
            if hasattr(dm_sig, "options_overlay") and dm_sig.options_overlay < -3:
                opts_warn = " (options bearish)"
            detail = f"Score {sc} {'STRONG' if sc >= 75 else 'GOOD'} — all conditions met — WR={wr:.0f} MFI={mfi:.0f}{opts_warn}"
            exit_txt = "PT +5% · BBW · 25d"
        elif tier == "SECONDARY":
            if sc >= 75:
                action = "▷ BUY HALF"
                detail = f"Score {sc} STRONG — core met, needs DD/streak"
            elif sc >= 60:
                action = "▷ BUY HALF"
            ...
            if mfi < 30:
                if wr < -50 and dd < -10:
                    action = "WATCH"
                    ac = BLUE
                    detail = f"Capitulation WR={wr:.0f} DD={dd:.1f}% — wait MFI>30"
                else:
                    action = "WAIT"
                    ac = GOLD
                    detail = f"MFI={mfi:.0f} weak — wait for accumulation"
            elif wr >= -30:
                action = "SKIP"
                ac = MUTED
                detail = f"WR={wr:.0f} not oversold"
            else:
                action = "FORMING"
                ac = GOLD
                detail = f"Score {sc} — building, watch for trigger"
            exit_txt = ""
    ...
    def _refresh_badges_dual(self, dm_sig):
        ...
        # Tier
        tier = dm_sig.tier
        tier_style = {"PRIMARY": "FRESH", "SECONDARY": "LATE", "NONE": "NEUTRAL"}.get(
            tier, "NEUTRAL"
        )
        self._badges["TIER"].set_badge(
            {"PRIMARY": "PRIMARY", "SECONDARY": "SECONDARY", "NONE": "NO ENTRY"}.get(
                tier, tier
            ),
            tier_style,
        )
        ...
        if not dm_sig.above_sma:
            self._badges["SMA"].set_badge(
                f"BELOW SMA {dm_sig.pct_from_sma:+.1f}%", "BULLISH"
            )
        else:
            self._badges["SMA"].set_badge(
                f"ABOVE SMA {dm_sig.pct_from_sma:+.1f}%", "BEARISH"
            )
    ...
    def _refresh_kpi(self):
        ...
        if ps:
            ...
            if ps.mfi_value is not None:
                mc = (
                    EM if ps.mfi_value >= 50 else (GREEN if ps.mfi_value >= 30 else RED)
                )
            ...
            if ps.daily_bias_pct:
                self._kpi["vs SMA"].set_delta(f"{ps.daily_bias_pct:+.1f}% vs SMA", bc)
        ...
        if (
            has_real_opts
            and ctx
            and hasattr(ctx, "expiry")
            and ctx.expiry.days_remaining < 99
        ):
