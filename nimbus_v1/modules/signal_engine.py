...
def compute_rs(symbol: str, lookback_days: int = 10) -> dict:
    ...
    rs_mkt = (stock_ret - market_ret) if (
        stock_ret is not None and market_ret is not None
    ) else None
    rs_sec = (stock_ret - sector_ret) if (
        stock_ret is not None and sector_ret is not None
    ) else None

    # Signal (pp-difference scale)
    if rs_sec is not None and rs_mkt is not None:
        if rs_sec > 0.03 and rs_mkt > 0.03:
            signal = "LEADING"
        elif rs_sec < -0.03 and rs_mkt < -0.03:
            signal = "LAGGING"
        else:
            signal = "NEUTRAL"
    else:
    ...
    return {
        "rs_vs_market": round(rs_mkt, 4) if rs_mkt is not None else None,
        "rs_vs_sector": round(rs_sec, 4) if rs_sec is not None else None,
        "rs_signal": signal,
    }
...
def enhanced_score(base_score: int, bonuses: dict, options_available: bool = True) -> int:
