...
_COLUMNS = [
    ("Symbol",   "symbol",          80),
    ("Added",    "added_at",        120),
    ("Entry",    "entry_price",     70),
    ("Stop",     "stop_price",      70),
    ("Target",   "target_price",    70),
    ("Score",    "live_score",      55),
    ("Tier",     "live_tier",       50),
    ("Size",     "live_size",       55),
    ("WR",       "live_wr",         55),
    ("MFI",      "live_mfi",        50),
    ("Verdict",  "live_verdict",    180),
    ("Notes",    "notes",           0),  # stretch
]
...
class WatchlistModel(QAbstractTableModel):
    ...
    def data(self, index: QModelIndex, role=Qt.ItemDataRole.DisplayRole):
        ...
        if col_key == "live_score":
            ...
            val = live.get("dm_mfi")
        elif col_key == "live_verdict":
            val = live.get("dm_verdict", "")
        else:
        ...
        if role == Qt.ItemDataRole.ForegroundRole:
            ...
            if col_key == "live_tier":
                ...
                return QBrush(QColor(sc.get(str(val), MUTED)))
            if col_key == "live_verdict" and val:
                v = str(val)
