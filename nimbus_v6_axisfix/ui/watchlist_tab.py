...
class WatchlistModel(QAbstractTableModel):
    ...
    def data(self, index: QModelIndex, role=Qt.ItemDataRole.DisplayRole):
        ...
        if col_key == "live_score":
            val = live.get("viability_score")
        elif col_key == "live_size":
            val = live.get("size_suggestion")
        elif col_key == "live_wr":
            val = live.get("wr_50")
        elif col_key == "live_state":
            val = live.get("position_state")
        else:
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if col_key == "added_at" and isinstance(val, str):
                return val[:16]  # trim to date + time
            return str(val)
        ...
        if role == Qt.ItemDataRole.ForegroundRole:
            ...
            if col_key == "live_size":
                sc = {"FULL": GREEN, "HALF": GOLD, "SKIP": RED, "ZERO": RED}
                return QBrush(QColor(sc.get(str(val), MUTED)))
            if col_key == "live_wr" and isinstance(val, (int, float)):
                c = EM if val >= -20 else (GOLD if val >= -50 else RED)
                return QBrush(QColor(c))
            if col_key == "live_state":
                sc = {
                    "RIDING_UPPER_BAND": EM, "FIRST_DIP": GOLD,
                    "MID_BAND_BROKEN": RED, "BELOW_MID": MUTED,
                }
                return QBrush(QColor(sc.get(str(val), MUTED)))
