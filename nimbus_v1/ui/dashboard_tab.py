...
class DashboardTab(QWidget):
    ...
    def _build_ui(self):
        ...
        br.setSpacing(4)
        self._badges = {}
        for k in ("DAILY", "VOL", "STATE", "W%R", "MFI", "REGIME"):
            b = Badge("--")
            br.addWidget(b)
            self._badges[k] = b
        br.addStretch(1)
    ...
    def _refresh_badges(self):
        ...
        if ps and ps.mfi_value is not None:
            ...
            if ps.mfi_diverge and ps.mfi_reliable:
                label = f"MFI {mv:.0f} ⚠ DIVERGE"
                state = "BEARISH"
            elif not ps.mfi_reliable:
                label = f"MFI {mv:.0f} (thin)"
                state = "NEUTRAL"
            else:
                state_map = {
                    "STRONG": "BULLISH",
                ...
                label = f"MFI {mv:.0f} {ps.mfi_state}"
                state = state_map.get(ps.mfi_state, "NEUTRAL")
            self._badges["MFI"].set_badge(label, state)
