...
def _walls(df: pd.DataFrame, spot: float, oi_pct: float = 75.0) -> Walls:
    ...
    resistance = float(above.loc[above["Call_OI"].idxmax(), "Strike"]) if not above.empty \
                 else (float(call_walls.loc[call_walls["Call_OI"].idxmax(), "Strike"]) if not call_walls.empty else None)

    below = put_walls[put_walls["Strike"] <= spot]
    support = float(below.loc[below["Put_OI"].idxmax(), "Strike"]) if not below.empty \
              else (float(put_walls.loc[put_walls["Put_OI"].idxmax(), "Strike"]) if not put_walls.empty else None)
    ...
    sentiment = (
        "Bullish"        if pcr >= 1.3 else
        "Mildly Bullish" if pcr >= 1.1 else
        "Neutral"        if pcr >= 0.9 else
        "Mildly Bearish" if pcr >= 0.7 else "Bearish"
    )
    return Walls(
...
def _gex(df: pd.DataFrame, spot: float, lot_size: int,
         r: float = 0.065, max_dte: int = 90) -> GEX:
    ...
    regime = (
        "Negative" if net < -abs_tot * 0.05 else
        "Positive" if net >  abs_tot * 0.05 else "Neutral"
    )
    ...
    return GEX(
        net_gex=round(net, 0), abs_gex=round(abs_tot, 0),
        regime=regime, hvl=hvl, by_expiry=by_expiry, spot=spot,
    )


def _expiry_ctx(df: pd.DataFrame, spot: float,
                max_pain: Optional[float]) -> ExpiryCtx:
    today    = datetime.date.today()
    ...
    if next_dte <= 1:
        pin_risk = "HIGH"
        warning  = f"Expiry TODAY — strong pin near {max_pain:,.0f}" if max_pain else "Expiry today"
    elif next_dte <= 2:
        pin_risk = "HIGH"
        warning  = f"Expiry in {next_dte}d — pin risk near {max_pain:,.0f}" if max_pain else f"Expiry in {next_dte}d"
    elif next_dte <= 4 and near_mp:
        pin_risk = "HIGH"
        warning  = f"Expiry {next_dte}d + spot within 1.2% of max pain {max_pain:,.0f} — HIGH pin risk"
    elif next_dte <= 4:
        pin_risk = "MODERATE"
        warning  = f"Expiry in {next_dte}d — watch for Thu/Fri compression near {max_pain:,.0f}" if max_pain else f"Expiry in {next_dte}d"
    elif near_mp:
        pin_risk = "MODERATE"
        warning  = f"Spot within 1.2% of max pain {max_pain:,.0f} — gravitational pull risk"
    else:
        pin_risk = "LOW"
        warning  = None
    ...
    return ExpiryCtx(
        ...
        pin_risk=pin_risk, warning=warning,
    )


def _regime_classify(gex: GEX, walls: Walls, expiry: ExpiryCtx,
                     spot: float) -> RegimeClass:
    """
...
def _viability(
    ...
    Scored conditions (bias, regime, structure, expiry, vol) determine conviction
    and sizing. They must influence the score, not unilaterally veto it, because
    these conditions rarely align perfectly and penalising marginal misses with a
    hard zero produces an unusable scanner.

    TIER 1 — HARD GATES (capped at SKIP/ZERO if failing):
      - 4H close riding upper BB(20, 1σ)
      - Williams %R(50) > -20
    TIER 2 — SCORED CONDITIONS (each contributes pts, partial credit where noted):
      Daily bias, GEX regime, structural room, expiry/DTE, vol state.
    See Qt roadmap Section 4 (Phase 5) for the full two-tier spec.
    """
    score     = 50
    checklist = []
    risk_notes = []
    ...
    def add(item, status, detail, impl):
        checklist.append(CheckItem(item=item, status=status, detail=detail, implication=impl))
    ...
    if regime == "Negative":
        score += 15
        add("GEX Regime", "pass", "Negative GEX",
            "Dealers amplify directional moves — momentum signals MORE reliable")
    elif regime == "Positive":
        score -= 15
        add("GEX Regime", "warn", f"Positive GEX ({ctx.gex.net_gex:+,.0f}M)",
        ...
        add("GEX Regime", "neutral", "Neutral GEX",
    ...
    if res and res_pct >= room_thresh:
        ...
        add("Structural Room", "neutral", "No resistance mapped", "Load options data for wall levels")

    # ── 4. Expiry / Max-Pain Risk Gate ────────────────────────────────────────
    dte = ctx.expiry.days_remaining
    if dte <= 1:
        score -= 25
        add("Expiry Gate", "fail",
    ...
    if pcr >= 1.5:
        score += 10
        add("PCR", "pass", f"PCR {pcr:.2f} — heavy put buying",
            "Heavy hedging activity → supports upside / floor below spot")
    elif 0.9 <= pcr <= 1.3:
        score += 5
        add("PCR", "pass", f"PCR {pcr:.2f} — balanced",
            "Balanced positioning — no extreme skew")
    elif pcr < 0.7:
        score -= 10
        add("PCR", "warn", f"PCR {pcr:.2f} — low / call-heavy",
    ...
    if ps is not None:
        ...
        if vs == "SQUEEZE":
            ...
            if ctx.regime.regime == "PINNING":
                ...
                risk("Late in volatility expansion with pinning regime — consider exiting")
            else:
                add("Vol State", "neutral",
                    f"EXPANDED ({ps.bb_width_pctl:.0f}th pctl)",
    ...
    if ps is not None and ps.wr_value is not None:
        if not ps.wr_in_momentum:
            ...
            if ps.wr_phase == "FRESH":
                ...
                risk(f"W%R late phase ({ps.wr_bars_since_cross50} bars since -50 cross) — reduce size")
    ...
    return Viability(
        ...
        sizing=sizing, checklist=checklist, risk_notes=risk_notes,
    )
