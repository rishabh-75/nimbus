...
NSE_LOT_SIZES: dict[str, int] = {
    ...
    "BAJFINANCE": 125, "WIPRO": 1500, "LT": 175, "MARUTI": 50,
    "ADANIENT": 625, "ADANIPORTS": 625, "TATAMOTORS": 1425,
    "TATASTEEL": 5500, "JSWSTEEL": 1350, "HINDALCO": 2150,
    "SUNPHARMA": 700, "DRREDDY": 125, "CIPLA": 650,
    "NTPC": 3000, "POWERGRID": 3375, "ONGC": 3850, "BPCL": 1800,
}
...
def _get_expiry_dates(session: requests.Session, cookies: dict,
                      symbol: str) -> list[str]:
    url = (
...
def _fetch_expiry(session: requests.Session, cookies: dict,
...
def _parse_rows(raw_rows: list, symbol: str, expiry: str) -> pd.DataFrame:
    ...
    for item in raw_rows:
        ...
        records.append({
            "Strike":         strike,
            "Expiry":         expiry,
            "UnderlyingValue": ce.get("underlyingValue") or pe.get("underlyingValue") or 0,
...
def get_price_4h(symbol: str, bars: int = 120) -> Tuple[pd.DataFrame, str]:
    ...
    try:
        ...
        if not raw.empty and "Close" in raw.columns and len(raw) >= 10:
            ...
            df_4h = raw.resample("4h", closed="right", label="right").agg(
            ...
            last_ts = raw.resample(
                "4h", closed="right", label="right"
            ).apply(lambda g: g.index[-1] if len(g) > 0 else None)
            df_4h.index = pd.DatetimeIndex([
                last_ts.get(t, t) for t in df_4h.index
            ])
...
def parse_uploaded_csv(file_obj) -> Tuple[Optional[pd.DataFrame], str]:
    ...
    try:
        ...
        rename = {
            "Strike Price": "Strike", "STRIKE": "Strike",
            "Expiry Date":  "Expiry", "EXPIRY DATE": "Expiry",
        }
...
def infer_spot(options_df: pd.DataFrame) -> Optional[float]:
    ...
    if "Strike" in options_df.columns:
        ...
        if oi_cols:
            oi = options_df[oi_cols].apply(pd.to_numeric, errors="coerce").sum(axis=1).fillna(0)
            total = oi.sum()
            if total > 0:
...
def _fetch_universe_from_nse() -> list[str]:
    ...
    for item in items:
        if isinstance(item, str):
            ...
            if sym and sym.replace("-", "").replace("&", "").isalpha() or sym in NSE_LOT_SIZES:
                symbols.add(sym)
...
NIFTY100_SYMBOLS = [
    ...
    "LT", "BAJFINANCE", "KOTAKBANK", "HCLTECH", "WIPRO", "MARUTI", "ONGC",
    "NTPC", "POWERGRID", "SUNPHARMA", "TATAMOTORS", "ADANIENT", "ADANIPORTS",
    "TATASTEEL", "JSWSTEEL", "HINDALCO", "BHARTIARTL", "TITAN", "ASIANPAINT",
    "BAJAJ-AUTO", "HEROMOTOCO", "DRREDDY", "CIPLA", "DIVISLAB", "NESTLEIND",
    "ULTRACEMCO", "GRASIM", "TECHM", "EICHERMOT", "INDUSINDBK", "BAJAJFINSV",
    "BPCL", "ITC", "COALINDIA", "M&M", "UPL", "TATACONSUM", "APOLLOHOSP",
    "HDFCLIFE", "SBILIFE", "BRITANNIA", "ZOMATO", "FINNIFTY", "MIDCPNIFTY",
]
