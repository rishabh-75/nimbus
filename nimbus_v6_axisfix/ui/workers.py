...
from modules.data import (
    get_price_4h, download_options, get_universe, NSE_LOT_SIZES,
)
...
class PriceWorker(QThread):
    ...
    def __init__(self, symbol: str, bb_period: int = 20,
                 bb_std: float = 1.0, wr_period: int = 50,
                 parent=None):
...
class ScanWorker(QThread):
    ...
    def __init__(self, symbols: list[str], min_viability: int = 50,
                 require_all_filters: bool = False, parent=None):
        super().__init__(parent)
    ...
    def run(self):
        logger.info(
            ...
            len(self.symbols), self.min_viability,
        )
        t0 = time.time()
        results = []
    ...
    def run(self):
        ...
        try:
            ...
            enriched = refresh_watchlist(
                self.entries,
                progress_cb=lambda done, total, sym: self.progress.emit(done, total, sym),
            )
            logger.info(
                "WatchlistWorker done: %d entries in %.1fs",
                len(enriched), time.time() - t0,
            )
