...
def _viability(
    ...
    # ── 9. MFI — daily volume-weighted momentum (Phase 2) ────────────────
    # Computed on daily-resampled frame inside compute_price_signals.
    # adv_cr gate prevents double-penalising thin volume (VSR already covers it).
    # Divergence overrides sizing to HALF rather than cutting score — maps to
    # how expiry pin risk and GEX sizecap already work in this codebase.
    if ps is not None and ps.mfi_value is not None:
        mv = ps.mfi_value

        if not ps.mfi_reliable:
            # Volume too thin — show in checklist but zero score impact
            add(
                "MFI",
                "neutral",
                f"MFI {mv:.0f} — low volume, signal unreliable",
                "Thin participation; VSR already accounts for volume quality",
            )

        elif ps.mfi_diverge:
            # Divergence = sizing warning, NOT a score cut
            # Sizing override applied below after all sections complete
            add(
                "MFI",
                "warn",
                f"MFI {mv:.0f} — BEARISH DIVERGENCE (price near high, money leaving)",
                "Volume not confirming price — position size reduced to HALF",
            )
            risk(f"MFI divergence at {mv:.0f} — distribution into price strength")

        elif ps.mfi_state == "STRONG":
            score += 5
            add(
                "MFI",
                "pass",
                f"MFI {mv:.0f} — strong buying pressure",
                "Volume confirming upside — healthy momentum",
            )

        elif ps.mfi_state == "RISING":
            if ps.wr_in_momentum:
                score += 3
                add(
                    "MFI",
                    "pass",
                    f"MFI {mv:.0f} — money flowing in (W%R confirms)",
                    "Volume and price momentum both aligned",
                )
            else:
                add(
                    "MFI",
                    "neutral",
                    f"MFI {mv:.0f} — rising but W%R not in zone",
                    "Volume improving but price momentum not confirmed yet",
                )

        elif ps.mfi_state == "FALLING":
            if ps.wr_trend == "falling":
                score -= 4
                add(
                    "MFI",
                    "warn",
                    f"MFI {mv:.0f} — money leaving (W%R falling too)",
                    "Both volume and price momentum deteriorating",
                )
                risk(f"MFI + W%R both falling — momentum losing conviction")
            else:
                add(
                    "MFI",
                    "neutral",
                    f"MFI {mv:.0f} — money flow softening",
                    "Volume weakening but price still holding — monitor",
                )

        elif ps.mfi_state == "WEAK":
            score -= 6
            add(
                "MFI",
                "fail",
                f"MFI {mv:.0f} — heavy selling pressure (3-bar confirmed)",
                "Sustained distribution — reduce size or avoid",
            )
            risk(f"MFI {mv:.0f} confirmed weak")

        else:
            add("MFI", "neutral", f"MFI {mv:.0f} — neutral flow", "No volume edge")

    # (existing hard overrides follow unchanged...)
    score = max(0, min(100, score))
    # ...label / color / sizing assignment...

    # ── MFI divergence sizing override (after label assignment) ──────────
    # Applied as a post-scoring override, same pattern as ctx.regime.sizecap
    if (
        ps is not None
        and getattr(ps, "mfi_diverge", False)
        and getattr(ps, "mfi_reliable", False)
    ):
        if sizing == "FULL":
            sizing = "HALF"
