...
class DashboardTab(QWidget):
    ...
    def _build_ui(self):
        ...
        for k in ("SPOT", "WR(30)", "MFI", "vs SMA", "PCR OI", "NET GEX", "TO EXPIRY", "VOL STATE"):
            t = KPITile(k); kr.addWidget(t); self._kpi[k] = t
        center.addLayout(kr)

        # ── Filing + Signal strip (combined) ──────────────────────────────
        fstrip = QFrame()
        fstrip.setFixedHeight(22)
        ...
        # Hidden label for entry reason (populated by on_dual_mode_updated)
        self._dm_entry_lbl = _t("", MUTED, 7)
        self._dm_entry_lbl.setVisible(False)
        center.addWidget(self._dm_entry_lbl)
