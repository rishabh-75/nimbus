    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if col_key == "dte" and isinstance(val, (int, float)):
                return f"{int(val)}d"
            if col_key == "dm_tier":
                return {"PRIMARY": "PRI", "SECONDARY": "SEC", "NONE": "—"}.get(
                    str(val), str(val)
                )
            ...
            if col_key == "dm_wr" and isinstance(val, (int, float)):
                return QBrush(
                    QColor(
                        EM
                        if val < -50
                        else (GREEN if val < -30 else (GOLD if val < -20 else MUTED))
                    )
                )
            if col_key == "dm_mfi" and isinstance(val, (int, float)):
                return QBrush(
                    QColor(EM if val >= 50 else (GREEN if val >= 30 else RED))
                )
            if col_key == "dm_pct_sma" and isinstance(val, (int, float)):
                return QBrush(
                    QColor(
                        RED
                        if val < -5
                        else (GOLD if val < -2 else (MUTED if val < 0 else EM))
                    )
                )
            if col_key == "dm_dd" and isinstance(val, (int, float)):
                return QBrush(
                    QColor(RED if val < -10 else (GOLD if val < -5 else MUTED))
                )
            if col_key == "dm_streak" and isinstance(val, (int, float)):
                return QBrush(
                    QColor(RED if val >= 4 else (GOLD if val >= 2 else MUTED))
                )
            if col_key == "gex_regime" and val == "ETF":
                return QBrush(QColor(BLUE))
