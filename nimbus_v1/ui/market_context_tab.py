...
class MarketContextTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self._sector_table = _styled_table(
            ...
            [155, 75, 75, 75, 75, 75, 100, 80, 120],  # all fixed
        )
    ...
    def _on_data(self, data: dict):
        ...
        for i, s in enumerate(sectors):
            ...
            # col 5 — Adj (weighted RS score — differs from RS when quality adj kicks in)
            adj_itm = QTableWidgetItem(
                f"{s['adj_score']:+.2f}" if s.get("adj_score") is not None else "—"
            )
            adj_itm.setForeground(QColor(EM if s.get("adj_score", 0) > 0 else RED))
            adj_itm.setTextAlignment(
                Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
            )
            self._sector_table.setItem(i, 5, adj_itm)
