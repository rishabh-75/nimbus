...
class DataManager(QObject):
    ...
    def refresh(self, symbol: str, fetch_options: bool = True):
        ...
        self._price_worker = PriceWorker(
            symbol=symbol, bb_period=self.bb_period,
            bb_std=self.bb_std, wr_period=self.wr_period,
        )
    ...
    @property
    def active_symbol(self) -> str: return self._active_symbol
    @property
    def price_df(self) -> Optional[pd.DataFrame]: return self._price_df
    @property
    def options_df(self) -> Optional[pd.DataFrame]: return self._options_df
    @property
    def context(self): return self._ctx
    @property
    def price_signals(self) -> Optional[PriceSignals]: return self._ps
    @property
    def spot(self) -> float: return self._spot
    @property
    def freshness(self) -> DataFreshnessState: return self._freshness

    def last_refresh_time(self):
        return datetime.datetime.fromtimestamp(self._last_refresh) if self._last_refresh else None
    def next_refresh_time(self):
        return datetime.datetime.fromtimestamp(self._last_refresh + _REFRESH_MS / 1000) if self._last_refresh else None
    ...
    def _run_analytics(self):
        ...
        if symbol in NSE_ETF_SYMBOLS:
            try:
                ...
                logger.info(
                    ...
                    symbol, ctx.viability.score,
                    ctx.viability.sizing, ctx.viability.label,
                )
        ...
        try:
            ...
            self._ctx = analyze(
                options_df, spot=spot, lot_size=self.lot_size,
                price_signals=self._ps, room_thresh=3.0,
            )
            logger.info(
    ...
    def _check_freshness(self):
        if self._last_refresh is None: return
    ...
    def _on_auto_refresh(self):
        if not self._active_symbol: return
        if not is_market_open():
            logger.info("Auto-refresh skipped: market closed"); return
        self.refresh(self._active_symbol, fetch_options=True)

    # ── PARAMETER UPDATES ─────────────────────────────────────────────────────

    def set_bb_period(self, val: int): self.bb_period = val
    def set_bb_std(self, val: float): self.bb_std = val
    def set_wr_period(self, val: int): self.wr_period = val
    def set_wr_threshold(self, val: float): self.wr_thresh = val
    def set_lot_size(self, val: int): self.lot_size = val
