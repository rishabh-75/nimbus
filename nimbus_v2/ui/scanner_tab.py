...
_COLUMNS = [
    ...
    ("Price", "last_price", 85),
    ("Score", "dm_score", 50),
    ("Verdict", "dm_verdict", 120),
    ("Size", "dm_sizing", 55),
    ("WR", "dm_wr", 50),
    ("MFI", "dm_mfi", 50),
    ("vs SMA", "dm_pct_sma", 65),
    ("DD%", "dm_dd", 55),
    ("Red", "dm_streak", 40),
    ("DTE", "dte", 45),
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if col_key == "gex_regime" and val == "ETF":
                return "ETF ✦"
            if col_key == "dm_score" and isinstance(val, (int, float)):
                entry = row_dict.get("dm_entry", False)
                tier = row_dict.get("dm_tier", "NONE")
                prefix = "▶" if entry and tier == "PRIMARY" else ("▷" if entry else "")
                return f"{prefix}{int(val)}"
            return str(val)

        if role == Qt.ItemDataRole.ForegroundRole:
            if col_key == "dm_verdict" and val:
                v = str(val)
                if "BUY FULL" in v: return QBrush(QColor(GREEN))
                if "BUY HALF" in v: return QBrush(QColor(GOLD))
                if "WATCH" in v: return QBrush(QColor(BLUE))
                if "WAIT" in v: return QBrush(QColor(GOLD))
                if "FORMING" in v: return QBrush(QColor(GOLD))
                if "AVOID" in v: return QBrush(QColor(RED))
            ...
            if col_key == "dm_streak" and isinstance(val, (int, float)):
                return QBrush(QColor(RED if val >= 5 else (GOLD if val >= 3 else MUTED)))
            if col_key == "gex_regime" and val == "ETF":
                return QBrush(QColor(BLUE))

        if role == Qt.ItemDataRole.BackgroundRole:
            if col_key == "dm_score" and isinstance(val, (int, float)):
                if val >= 70: return QBrush(QColor(16, 185, 129, 30))
                elif val >= 50: return QBrush(QColor(245, 158, 11, 25))
            if col_key == "dm_score" and row_dict.get("dm_entry"):
                tier = row_dict.get("dm_tier", "NONE")
                alpha = 45 if tier == "PRIMARY" else 25
                return QBrush(QColor(16, 185, 129, alpha))

        if role == Qt.ItemDataRole.FontRole:
            if col_key in ("dm_score", "dm_sizing", "dm_tier", "symbol"):
        ...
        if role == Qt.ItemDataRole.TextAlignmentRole:
            if col_key in ("last_price", "dm_score", "dm_wr", "dm_mfi",
                           "dm_pct_sma", "dm_dd", "dm_streak", "dte"):
...
class ScannerTab(QWidget):
    ...
    def _on_run(self):
        ...
        self._scan_worker = ScanWorker(
            symbols=symbols,
            min_score=self._min_score_slider.value(),
    ...
    def _on_filing_enriched(self, symbol: str, updated: dict):
        for row in self._model._data:
            if row.get("symbol") == symbol:
                row.update(updated)
                break
        # Don't re-sort or reset on every row — batch it in _on_filings_done
