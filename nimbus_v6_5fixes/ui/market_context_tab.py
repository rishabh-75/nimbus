...
def _fetch_sectors():
    ...
    from modules.sector_map import SECTOR_TICKERS, SECTOR_NAMES, MARKET_TICKER, MARKET_FALLBACK, IT_TICKERS
    ...
    for t in SECTOR_TICKERS:
        ...
        # IT fallback
        if ret is None and t in IT_TICKERS:
            for alt in IT_TICKERS:
                if alt != t:
                    ret = _get_return(alt, 10)
                    if ret is not None: break
        if ret is not None and mkt is not None and mkt != 0:
            rows.append({"name": SECTOR_NAMES.get(t, t), "return": round(ret*100, 1),
                         "vs_market": round((ret - mkt)*100, 1)})
        elif SECTOR_NAMES.get(t, "").lower() == "it":
            rows.append({"name": "IT", "return": 0, "vs_market": 0})
...
class MarketContextTab(QWidget):
    ...
    def _build_ui(self):
        ...
        # FII
        fii_frame = QFrame(); fii_frame.setMaximumWidth(320)
        fc = QVBoxLayout(fii_frame); fc.setSpacing(2); fc.setContentsMargins(0,0,0,0)
        ...
        fl.addWidget(fii_frame)

        # DII
        dii_frame = QFrame(); dii_frame.setMaximumWidth(320)
        dc = QVBoxLayout(dii_frame); dc.setSpacing(2); dc.setContentsMargins(0,0,0,0)
    ...
    def _on_data(self, data):
        ...
        for i, s in enumerate(sectors):
            ...
            vs = s["vs_market"]
            if vs > 0.5: txt, col = f"+{vs:.1f}pp", EM
            elif vs < -0.5: txt, col = f"{vs:.1f}pp", RED
            else: txt, col = "In line", MUTED
            ...
            self._sector_table.setItem(i, 3, QTableWidgetItem(""))
