...
def _sector_tooltip(s: dict) -> str:
    ...
    lines = [
        ...
        f"Conviction: {s.get('conviction_score', 0):+.2f}  (RS+entry+MFI)",
        (
            f"MFI        : {s['mfi_value']:.0f}  [{s.get('mfi_state', '—')}]"
            if s.get("mfi_value") is not None
            else f"MFI        : —  [{s.get('mfi_state', '—')}]"
        ),
        f"Diverge    : {'⚠ YES' if s.get('mfi_diverge') else 'No'}",
        f"Entry      : {s.get('entry_label', '—')} ({s.get('entry_score', '—')}/100)",
        "─────────────────────────",
...
class MarketContextTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self._sector_table = _styled_table(
            ...
            [150, 70, 70, 70, 70, 70, 105, 95, 160],  # all fixed
        )
        ...
        self._event_table = _styled_table(
            ...
            [120, 75, 0, 180, 120],  # 0 = Event stretches
        )
    ...
    def _on_data(self, data: dict):
        ...
        for i, s in enumerate(sectors):
            ...
            # col 4 — RS (use vs_market = simple 10D return differential, intuitive)
            rs_itm = QTableWidgetItem(
                f"{s['vs_market']:+.1f}%" if s.get("vs_market") is not None else "—"
            )
            rs_itm.setForeground(
                QColor(
                    EM
                    if s.get("vs_market", 0) > 2
                    else (RED if s.get("vs_market", 0) < -2 else GOLD)
                )
            )
            rs_itm.setTextAlignment(
                Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
            )
            self._sector_table.setItem(i, 4, rs_itm)

            # col 5 — Conv (conviction score: RS + entry viability + MFI)
            conv = s.get("conviction_score")
            conv_itm = QTableWidgetItem(f"{conv:+.1f}" if conv is not None else "—")
            conv_itm.setForeground(
                QColor(EM if (conv or 0) > 2 else (GOLD if (conv or 0) > 0 else RED))
            )
            conv_itm.setTextAlignment(
                Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
            )
            self._sector_table.setItem(i, 5, conv_itm)

            ##
            mfi_val = s.get("mfi_value")
            mfi_state = s.get("mfi_state", "NEUTRAL")
            mfi_div = s.get("mfi_diverge", False)

            if mfi_val is not None:
                prefix = "⚠ " if mfi_div else ""
                mfi_text = f"{prefix}{mfi_val:.0f} {mfi_state}"
            else:
                mfi_text = "—"

            mfi_itm = QTableWidgetItem(mfi_text)
            mfi_itm.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            mfi_color = {
                "STRONG": EM,
                "RISING": GREEN,
                "NEUTRAL": MUTED,
            ...
            if mfi_div:
                mfi_color = RED
            mfi_itm.setForeground(QColor(mfi_color))
            self._sector_table.setItem(i, 6, mfi_itm)
            ##
            ...
            self._sector_table.setItem(i, 7, e_itm)
            ...
            self._sector_table.setItem(i, 8, rot_itm)
    ...
    def _refresh_events(self):
        ...
        for i, ev in enumerate(upcoming):
            edate = QTableWidgetItem(ev["date"])
            edate.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self._event_table.setItem(i, 0, edate)
