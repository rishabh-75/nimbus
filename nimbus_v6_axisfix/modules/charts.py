...
def main_chart(
    ...
    if price_df is None or price_df.empty:
        fig = go.Figure()
        fig.update_layout(
            paper_bgcolor=BG, plot_bgcolor=SURFACE, height=640,
    ...
    if has_bb:
        ...
        fig.add_trace(go.Scatter(
            ...
            connectgaps=True,
            hovertemplate="BB Up: %{y:.1f}<extra></extra>", showlegend=False,
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=xs, y=lower,
            line=dict(color=BB_LINE, width=1.2),
            connectgaps=True,
            hovertemplate="BB Lo: %{y:.1f}<extra></extra>", showlegend=False,
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=xs, y=mid,
            line=dict(color=BB_MID, width=0.9, dash="dot"),
            connectgaps=True,
            hovertemplate="BB Mid: %{y:.1f}<extra></extra>", showlegend=False,
        ), row=1, col=1)

    # ── Row 1: Daily SMA line (if available) ──────────────────────────────────
    if ps and ps.daily_sma:
        sma_val = ps.daily_sma
        sma_color = EM if ps.daily_bias == "BULLISH" else (RED if ps.daily_bias == "BEARISH" else GOLD)
        fig.add_shape(
            type="line", xref="paper", yref="y1",
            x0=0, x1=1, y0=sma_val, y1=sma_val,
            line=dict(color=sma_color, width=1.0, dash="dash"),
        )
        fig.add_annotation(
            xref="paper", yref="y1", x=0.01, y=sma_val,
    ...
    if has_vol:
        ...
        fig.add_trace(go.Bar(
            x=xs, y=df["Volume"].fillna(0).tolist(),
    ...
    if has_wr:
        ...
        fig.add_trace(go.Scatter(
            ...
            connectgaps=True,
            hovertemplate="W%%R: %{y:.1f}<extra></extra>", showlegend=False,
        ), row=3, col=1)

        # Momentum zone fill (above -20 = green)
        above = [max(v, -20) for v in wr_vals]
        fig.add_trace(go.Scatter(
            x=xs + xs[::-1], y=above + [-20] * len(xs),
        ...
        for y_val, color, lbl in [(-20, EM, "-20"), (-80, RED, "-80")]:
            fig.add_annotation(
                xref="paper", yref="y3", x=1.01, y=y_val, text=lbl,
                showarrow=False,
    ...
    fig.update_layout(
        ...
        title=dict(
            text=(f"<b style='color:{WHITE}'>{symbol}</b>"
        ...
        xaxis=xax, xaxis2=xax, xaxis3=xax,
        yaxis=dict(gridcolor=BORDER, color=WHITE, tickformat=",.0f",
                   tickfont=dict(size=9), side="right", showline=False),
        yaxis2=dict(gridcolor="rgba(0,0,0,0)", color=MUTED, tickformat=".2s",
                    tickfont=dict(size=7, color=MUTED), side="right", showgrid=False),
        yaxis3=dict(gridcolor=BORDER, color=MUTED, tickformat=".0f",
                    tickfont=dict(size=8, color=MUTED), side="right",
                    range=[-105, 5], tickvals=[-20, -50, -80]),
    )
    return fig
...
def _add_wall_lines(fig: go.Figure, ctx: OptionsContext) -> None:
    levels = [
        (ctx.walls.resistance, RED,    "dash",    f"R  {ctx.walls.resistance:,.0f}"  if ctx.walls.resistance else None),
        (ctx.walls.support,    EM,     "dash",    f"S  {ctx.walls.support:,.0f}"     if ctx.walls.support    else None),
        (ctx.walls.max_pain,   GOLD,   "dot",     f"MP {ctx.walls.max_pain:,.0f}"    if ctx.walls.max_pain   else None),
        (ctx.gex.hvl,          VIOLET, "dashdot", f"HVL {ctx.gex.hvl:,.0f}"          if ctx.gex.hvl          else None),
    ]
    for y_val, color, dash, label in levels:
        ...
        fig.add_annotation(
            xref="paper", yref="y1", x=1.01, y=y_val,
            text=f"<b>{label}</b>",
            showarrow=False,
...
def gex_expiry_bar(ctx: OptionsContext) -> go.Figure:
    ...
    if not data:
        ...
        colors = ["rgba(38,166,154,0.75)" if v >= 0 else "rgba(239,68,68,0.75)" for v in vals]
        fig.add_trace(go.Bar(
    ...
    fig.update_layout(
        ...
        margin=dict(l=8, r=8, t=24, b=8), height=150,
        ...
        yaxis=dict(gridcolor=BORDER, color=MUTED, tickformat=",.3s",
                   zerolinecolor=BORDER, tickfont=dict(size=8)),
        bargap=0.2,
    )
    return fig
