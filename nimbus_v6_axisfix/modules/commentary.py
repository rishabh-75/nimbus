...
def get_commentary(
    ...
    return _rule_based(ctx, ps, bb, wr)


def _build_context(ctx: OptionsContext, ps: Optional[PriceSignals],
                   bb: dict, wr: dict, symbol: str) -> str:
...
def _rule_based(ctx: OptionsContext, ps: Optional[PriceSignals],
                bb: dict, wr: dict) -> dict:
    g, w, e, v, r = ctx.gex, ctx.walls, ctx.expiry, ctx.viability, ctx.regime

    # ── GEX line ──────────────────────────────────────────────────────────────
    if g.regime == "Negative":
        gex_line = (f"GEX Negative ({g.net_gex:,.0f}M) → Dealers amplify moves "
                    f"→ BB momentum MORE reliable → Full size")
    elif g.regime == "Positive":
        gex_line = (f"GEX Positive ({g.net_gex:,.0f}M) → Pins toward {w.max_pain:,.0f} "
                    f"→ Momentum faces headwinds → Size down 50%")
    else:
        gex_line = ("GEX Neutral → No strong dealer bias "
                    "→ Trade on walls + price action → Standard sizing")

    # ── Wall line ─────────────────────────────────────────────────────────────
    if w.resistance and w.resistance_pct >= 2.0:
        wall_line = (f"Resistance {w.resistance:,.0f} ({w.resistance_pct:+.1f}%) | "
                     f"Support {w.support:,.0f} ({w.support_pct:+.1f}%) "
                     f"→ Adequate structural room → Target resistance, stop below support")
    elif w.resistance:
        wall_line = (f"Resistance {w.resistance:,.0f} ({w.resistance_pct:+.1f}%) — CRAMPED "
    ...
    if wr_val is not None:
        if not wr_zone:
            wr_line = (f"W%R {wr_val:.1f} → Below -20 → NOT in momentum zone "
                       f"→ Do NOT enter until W%R reclaims -20")
        elif wr_phase == "FRESH":
            wr_line = (f"W%R {wr_val:.1f} → FRESH momentum ({bars}b since -50 cross) "
                       f"→ Early in the move — best entry window → Proceed")
        elif wr_phase == "DEVELOPING":
            wr_line = (f"W%R {wr_val:.1f} → DEVELOPING ({bars}b since cross) "
                       f"→ Mid-phase momentum — still valid → Monitor for exhaustion")
        else:
            wr_line = (f"W%R {wr_val:.1f} → LATE phase ({bars}b since -50 cross) "
    ...
    if ps:
        bias_line = (f"Daily {bias_str} ({bias_pct} vs SMA) · Vol {vs_str} "
    ...
    if ps and ps.position_state == "MID_BAND_BROKEN":
        verdict = (f"EXIT signal. Price closed below 20-SMA — momentum leg complete. "
                   f"Close all positions. Re-enter only after W%R resets below -50 and bounces.")
    elif ps and ps.position_state == "FIRST_DIP":
        verdict = (f"Partial exit. First close below upper band at {ps.last_close:,.0f}. "
    ...
    # Daily bearish — no longs
        ...
        verdict = (f"No new longs. Daily price is BELOW 20-SMA ({ps.daily_bias_pct:+.1f}%). "
    ...
    # WR gate — not in momentum zone
        ...
        if score < 40 or sizing == "SKIP":
            verdict = (f"No entry. W%R {wr_str} not in momentum zone AND conditions unfavorable. "
                       f"Wait for W%R > -20. Check again next session.")
        elif g.regime == "Positive":
            verdict = (f"Wait. W%R {wr_str} needs > -20. GEX Positive also pins toward {mp_str}. "
                       f"Two reasons to stay out. Alert at W%R = -20.")
        else:
            verdict = (f"Setup building but W%R {wr_str} not yet at -20. "
    ...
    # SKIP / AVOID
        ...
        note_str = " | ".join(notes)
        verdict  = (f"Skip. {note_str + '. ' if note_str else ''}"
                    f"Conditions do not support a momentum trade. Wait for next cycle.")

    # HALF size
    elif sizing == "HALF":
        reasons = []
        if g.regime == "Positive": reasons.append(f"GEX Positive (→ {mp_str})")
        if e.pin_risk != "LOW":    reasons.append(f"pin risk ({dte}d)")
        if ps and wr_phase == "LATE": reasons.append("late W%R phase")
        why = " + ".join(reasons) if reasons else "mixed conditions"
        verdict = (f"Half size only ({why}). W%R in zone, target {res_str}, stop below {sup_str}. "
                   f"Take partial at midpoint, trail the rest.")

    # FULL size
    else:
        phase_note = f"Fresh W%R momentum ({bars}b). " if wr_phase == "FRESH" else ""
        squeeze_note = "Vol squeeze breakout in play. " if ps and ps.vol_state == "SQUEEZE" else ""
        verdict = (f"Full size. {phase_note}{squeeze_note}"
                   f"Regime {r.regime}, adequate room to {res_str}. "
                   f"Enter on 4h BB confirmation. Hard stop below {sup_str}.")

    return dict(
        gex_line=gex_line, wall_line=wall_line,
        expiry_line=exp_line, wr_line=wr_line,
        bias_line=bias_line,
        verdict=verdict, sizing=sizing,
    )
