...
def _build_dual_checklist(dm_sig, ctx=None):
    ...
    if dm_sig.tier == "PRIMARY":
        checks.append(("pass", "▶ PRIMARY", dm_sig.entry_reason, "FULL size, 3% PT"))
    elif dm_sig.tier == "SECONDARY":
        checks.append(("pass", "▷ SECONDARY", dm_sig.entry_reason, "HALF size, 3% PT"))
    else:
        missing = []
    ...
    def on_dual_mode_updated(self, symbol: str, dm_sig):
        ...
        if dm_sig.tier == "PRIMARY":
            ...
            verdict = (
                ...
                f"Size: FULL. Exit: +3% PT or BBW contraction. Max hold: 30d."
            )
        elif dm_sig.tier == "SECONDARY":
            vrgb = "245,158,11"; vc = GOLD
            verdict = (
                f"SECONDARY ENTRY: {dm_sig.entry_reason}. "
                f"Size: HALF. Exit: +3% PT or BBW contraction. Max hold: 30d."
            )
        elif dm_sig.core_met:
            vrgb = "245,158,11"; vc = GOLD
            missing = []
            if dm_sig.dd_from_high > -5: missing.append(f"DD={dm_sig.dd_from_high:.1f}% (need <-5%)")
            if dm_sig.red_streak < 3: missing.append(f"{dm_sig.red_streak}d red (need ≥3)")
