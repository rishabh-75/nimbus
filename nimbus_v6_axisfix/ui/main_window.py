...
from ui.theme import BG, SURFACE, BORDER, EM, RED, GOLD, MUTED, WHITE, FONT_MONO, QSS
...
from ui.kite_manager import KiteSessionManager, TickerWorker, TickAggregator
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # ── Kite session manager (Phase 7) ────────────────────────────────────
        self._kite_mgr = KiteSessionManager(parent=self)
        self._ticker_worker: Optional[TickerWorker] = None
        self._tick_agg = TickAggregator()
        self._kite_connected = False

        self._kite_mgr.session_ready.connect(self._on_kite_session_ready)
        self._kite_mgr.session_invalid.connect(self._on_kite_session_invalid)
    ...
    def closeEvent(self, event):
        """Persist state and stop Kite ticker on close."""
        self._save_state()
        if self._ticker_worker and self._ticker_worker.isRunning():
            self._ticker_worker.stop()
            self._ticker_worker.wait(2000)
        super().closeEvent(event)

    # ──────────────────────────────────────────────────────────────────────────
    # KITE INTEGRATION (Phase 7)
    # ──────────────────────────────────────────────────────────────────────────

    def _on_kite_session_ready(self, access_token: str):
        """Valid Kite session — start WebSocket ticker."""
        logger.info("Kite session ready — starting ticker")
        self._set_status("LIVE", "Kite session valid — connecting ticker…")
        ...
        # Start new ticker
        self._ticker_worker = TickerWorker(
            api_key=self._kite_mgr.api_key,
            access_token=access_token,
        )
        self._ticker_worker.tick_received.connect(self._on_tick_received)
        self._ticker_worker.connected.connect(self._on_ticker_connected)
        self._ticker_worker.disconnected.connect(self._on_ticker_disconnected)
        self._ticker_worker.start()

    def _on_kite_session_invalid(self, msg: str):
        """No valid Kite session — yfinance fallback (silent)."""
        logger.info("Kite unavailable (%s) — yfinance polling active", msg)
        self._kite_connected = False
    ...
    def _on_tick_received(self, ticks: list):
        """Process real-time ticks from Kite."""
        for tick in ticks:
            ...
            symbol = self.data_mgr.active_symbol
            self.data_mgr._spot = ltp
            self.data_mgr.spot_updated.emit(symbol, ltp)

            # Aggregate into 4H bars
            completed = self._tick_agg.add_tick(tick)
            if completed:
                # A 4H bar completed — trigger full pipeline refresh
                logger.info("4H bar completed from ticks — refreshing pipeline")
                self.data_mgr.refresh(symbol, fetch_options=True)

        # Update status bar with live price
    ...
    def _on_kite_reauth_clicked(self):
        ...
        url = self._kite_mgr.start_reauth()
        if not url:
            return
        ...
        if ok and token.strip():
            self._kite_mgr.complete_reauth(token.strip())
