...
OOS: Sharpe 4.17, Win 85.3%, Avg +2.44%, N=75

ENTRY (all on DAILY bars):
  Core:    WR(30) < -30 AND close < SMA(20) AND MFI(14) > 30
  Primary: + drawdown > 5% from 50d high AND ≥3 consecutive red days AND vol > 0.5x avg
  Secondary: core only (more signals, lower conviction)

EXIT:
  BBW contraction (BBW slope < 0 AND above SMA) OR +3% profit target
  Max hold: 30 days
...
RED_STREAK_MIN = 3    # consecutive red days
VOL_MIN_RATIO = 0.5   # volume vs 20d average
PROFIT_TARGET = 3.0   # exit at +3%
MAX_HOLD = 30          # max hold in trading days
MIN_DAILY_BARS = 35
...
def _score(sig):
    ...
    if sig.red_streak >= 5:   s += 6
    elif sig.red_streak >= 3: s += 3
