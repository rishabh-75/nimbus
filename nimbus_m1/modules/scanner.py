...
Entry conditions (all must pass for all_filters_pass=True):
  a. Daily close riding upper BB(20,1σ) — RIDING_UPPER only (not FIRST_DIP)
  b. W%R(50) > -20
...
from modules.data import get_price_daily, download_options, infer_spot, NSE_LOT_SIZES
...
def _analyze_symbol_inner(symbol: str) -> Optional[dict]:
    ...
    # ── Price data ────────────────────────────────────────────────────────
    price_df, p_msg = get_price_daily(symbol, days=365)
    ...
    # ── Dual-mode signal (runs alongside legacy scoring) ──────────────
    dm_score = score
    dm_label = label
    dm_sizing = size
    dm_tier = "NONE"
    ...
    try:
        ...
        if dm_sig.data_sufficient:
            dm_score = dm_sig.dual_score
            dm_label = dm_sig.dual_label
            dm_sizing = dm_sig.dual_sizing
            dm_tier = dm_sig.tier
            dm_entry = dm_sig.entry_triggered
            dm_entry_reason = dm_sig.entry_reason
            dm_wr = dm_sig.wr_30
            dm_adx = dm_sig.adx_14
            dm_pct_sma = dm_sig.pct_from_sma
            dm_mfi = dm_sig.mfi
            dm_dd = dm_sig.dd_from_high
            dm_streak = dm_sig.red_streak
            dm_bbw_slope = dm_sig.bbw_slope
    except Exception as exc:
        logger.debug("DualMode scan %s: %s", symbol, exc)
    ...
    result = {
        ...
        "mfi_value": round(ps.mfi_value, 1) if ps.mfi_value is not None else None,
        "mfi_state": ps.mfi_state,  # STRONG/RISING/NEUTRAL/FALLING/WEAK
        "mfi_diverge": ps.mfi_diverge,  # True = bearish divergence, size HALF
        # Dual-mode fields
        "dm_score": dm_score,
        "dm_label": dm_label,
        "dm_sizing": dm_sizing,
        "dm_tier": dm_tier,
        "dm_entry": dm_entry,
        "dm_entry_reason": dm_entry_reason,
        "dm_wr": round(dm_wr, 1) if dm_wr is not None else None,
        "dm_adx": round(dm_adx, 1),
        "dm_pct_sma": round(dm_pct_sma, 1),
        "dm_mfi": round(dm_mfi, 1),
        "dm_dd": round(dm_dd, 1),
        "dm_streak": dm_streak,
        "dm_bbw_slope": round(dm_bbw_slope, 3),
        "dm_mode": "MR",  # backward compat
        "dm_segment": "UNIFIED",
    }

    # Override short_reason with unified mean-reversion context
    if dm_entry and dm_entry_reason:
        result["short_reason"] = dm_entry_reason
    elif dm_score >= 60:
        result["short_reason"] = (
            f"MR: WR={dm_wr:.0f} SMA{dm_pct_sma:+.1f}% MFI={dm_mfi:.0f}"
            if dm_wr is not None else short_reason
        )

    # Compute verdict — single actionable phrase
    result["dm_verdict"] = _compute_verdict(result)

    return result


def _compute_verdict(r: dict) -> str:
    """Generate a single actionable verdict from scanner row data."""
    tier = r.get("dm_tier", "NONE")
    score = r.get("dm_score", 0)
    mfi = r.get("dm_mfi", 0)
    wr = r.get("dm_wr") or 0
    dd = r.get("dm_dd", 0)
    streak = r.get("dm_streak", 0)
    sizing = r.get("dm_sizing", "SKIP")
    opt_ov = r.get("dm_entry", False)

    # TRAP check
    if score <= 30:
        return "⚠ AVOID — TRAP or weak"

    # Entry triggered
    if tier == "PRIMARY":
        if score >= 75:
            return f"▶ BUY FULL — strong setup"
        return f"▶ BUY FULL — {score} good"
    elif tier == "SECONDARY":
        if score >= 75:
            return f"▷ BUY HALF — strong, needs confirmation"
        if score >= 60:
            return f"▷ BUY HALF — decent setup"
        return f"▷ HALF — marginal"

    # No entry — explain what's missing
    if mfi < 30:
        if wr < -50 and dd < -10:
            return f"WATCH — capitulation, wait MFI>{30}"
        if wr < -30:
            return f"WAIT — oversold, MFI={mfi:.0f} weak"
        return f"SKIP — not oversold, MFI weak"

    if wr >= -30:
        return f"SKIP — WR={wr:.0f} not oversold"

    # Core met but not triggered (shouldn't happen if MFI>=30 and WR<-30 and below SMA)
    if score >= 60:
        return f"FORMING — {score}, watch for entry"

    return f"SKIP — score {score} too low"
