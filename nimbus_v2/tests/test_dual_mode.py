...
def test_exit():
    ...
    def t_profit():
        sig = DualModeSignal(close=1030); reason = check_exit(sig, 1000, 5)
        assert reason == "PROFIT_TARGET"
    _run("profit_target_3pct", t_profit)
    def t_bbw():
    ...
    def t_max():
        sig = DualModeSignal(close=990); reason = check_exit(sig, 1000, 30)
        assert reason == "MAX_HOLD"
    _run("max_hold_30d", t_max)
    def t_hold():
        sig = DualModeSignal(close=1010); reason = check_exit(sig, 1000, 3)
